...
import { clearTenantData, getActiveTenantId } from '../services/firestoreService';
...
export default function SettingsAndAccounts() {
  ...
  const handleResetPassword = async (id, username) => {
    const newPass = window.prompt(`Masukkan password baru untuk akun "${username}":`);
    if (newPass && newPass.trim().length >= 4) {
      try {
        await updateUserAccount(id, { password: newPass.trim() });
        alert(`Password untuk "${username}" berhasil diubah menjadi: ${newPass.trim()}`);
        loadAccountsAndSantri();
      } catch (err) {
        alert('Gagal memperbarui password akun');
      }
          ...
          <div className="p-6 border-b border-[#E4E4E7] flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              ...
              <p className="text-slate-400 text-[11px] mt-0.5">
                Kelola akun pengguna, reset password instan, dan data otomatis terisolasi di Firestore per-tenant (tidak tercampur).
              </p>
            </div>
            <button
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-[#FAFAF8] border-b border-[#E4E4E7] text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                  ...
                  <th className="py-3.5 px-4">Santri Asuh</th>
                  <th className="py-3.5 px-4 text-center">Aksi Pengurus & Reset</th>
                </tr>
              </thead>
                ...
                {accounts.map((acc) => {
                  ...
                  return (
                    <tr key={acc.id} className="hover:bg-slate-50/70">
                      ...
                      <td className="py-3.5 px-4 font-mono text-slate-600 font-semibold">{acc.username}</td>
                      ...
                      <td className="py-3.5 px-4">
                        <span className={`px-2 py-0.5 rounded font-bold text-[10px] ${
                          ...
                          {managedCount > 0 ? `${managedCount} Santri` : 'Bebas'}
                        </span>
                      </td>
                      <td className="py-3.5 px-4 text-center">
                        <div className="flex items-center justify-center gap-1.5">
                          ...
                          <button
                            ...
                            className="px-2 py-1 bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 rounded-lg text-[10px] font-bold transition-colors flex items-center gap-1"
