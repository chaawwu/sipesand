...
export default function PocketAndCash({ onOpenNfcModal, currentUser }) {
  ...
  return (
    <div className="space-y-6 text-xs font-sans">
      ...
      {/* Top Banner Devisi Uang Saku Woot Style */}
      <div className="bg-white p-5 rounded-3xl border border-[#E4E4E7] shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <h2 className="text-base font-black text-[#18181B]">Manajemen Uang Saku & POS Kasir</h2>
            {userRole === 'PENGURUS_SAKU' && Array.isArray(managedIds) && managedIds.length > 0 && (
              <span className="px-2.5 py-0.5 rounded-full text-[10px] font-black bg-blue-50 text-[#0B5FFF] border border-blue-200">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="px-4 py-2 bg-zinc-100 hover:bg-zinc-200 text-zinc-700 font-bold rounded-2xl transition-all flex items-center gap-1.5 border border-zinc-200 disabled:opacity-50 text-xs"
          >
            <CreditCard className="w-4 h-4 text-[#0B5FFF]" />
            <span>ID Card KTS</span>
          </button>

          {isNfcEnabled && (
            <button
              onClick={onOpenNfcModal}
              className="px-4 py-2 bg-[#0B5FFF] hover:bg-blue-700 text-white font-black rounded-2xl shadow-sm transition-all flex items-center gap-1.5 text-xs uppercase tracking-wide"
            >
              <Radio className="w-4 h-4 stroke-[2.5]" />
              <span>Tap Smart NFC</span>
            </button>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 bg-white rounded-3xl border border-[#E4E4E7] shadow-sm p-6 space-y-5">
          <div className="flex items-center justify-between pb-3 border-b border-zinc-100">
            <div>
              <h3 className="font-black text-sm text-[#18181B]">Input Setor & Tarik Uang Saku Santri</h3>
              <p className="text-[11px] text-zinc-400 font-medium mt-0.5">Uang cash santri dipegang dan dicatat langsung oleh pengurus</p>
            </div>
            <span className="text-[11px] font-black text-[#0B5FFF] bg-blue-50 px-2.5 py-0.5 rounded-full border border-blue-200">
          ...
          <div className="grid grid-cols-3 gap-2 bg-zinc-100 p-1.5 rounded-2xl">
            <button
              ...
              className={`py-2 rounded-xl font-bold transition-all text-xs ${
                txType === 'WITHDRAW' ? 'bg-[#18181B] text-white shadow-sm font-extrabold' : 'text-zinc-600 hover:text-zinc-900'
              }`}
            >
              Tarik Tunai Santri
            </button>
            <button
              ...
              className={`py-2 rounded-xl font-bold transition-all text-xs ${
                txType === 'TOPUP' ? 'bg-[#0B5FFF] text-white shadow-sm font-extrabold' : 'text-zinc-600 hover:text-zinc-900'
              }`}
            >
              Setor / Top-Up Saku
            </button>
            <button
              ...
              className={`py-2 rounded-xl font-bold transition-all text-xs ${
                txType === 'PURCHASE' ? 'bg-[#FF7A00] text-white shadow-sm font-extrabold' : 'text-zinc-600 hover:text-zinc-900'
              }`}
            >
              Belanja Kebutuhan
            </button>
