...
import MobileTenantSwitcherModal from './MobileTenantSwitcherModal';
  ...
  return (
    <header className="sticky top-0 z-30 bg-white/95 backdrop-blur-sm border-b border-slate-200 px-4 sm:px-6 py-2.5 sm:py-3 flex items-center justify-between gap-3 text-xs font-sans">
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
        ...
        {/* Tenant Indicator (Locked per-tenant, but allows explicit switching if needed) */}
        {isTenantInstance || (activeTenant && activeTenant !== 'master') ? (
          <button 
            onClick={() => setIsSwitcherOpen(true)}
            className="flex items-center gap-1.5 px-2.5 py-1.5 bg-emerald-50/90 hover:bg-emerald-100 border border-emerald-200 rounded-lg text-emerald-900 shadow-2xs transition-colors cursor-pointer"
            title="Klik untuk melihat atau mengganti pondok pesantren"
          >
      ...
      {/* Modal Ganti Lembaga Pondok Multi-Tenant */}
