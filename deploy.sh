...
# 1. Install root runtime dependencies used by the production frontend server
echo "📦 [1/5] Menginstal dependensi runtime root..."
npm install --production=false

# 2. Install Backend Dependencies & Run Database Migrations
echo "📦 [2/5] Menginstal dependensi Backend & Sinkronisasi Database Prisma..."
cd backend
npm install --production=false
npx prisma generate
npx prisma db push
...
echo "🎨 [3/5] Melakukan build aset produksi Frontend..."
cd frontend
npm install --production=false
npm run build
cd ..
...
# 5. Start / Reload with PM2 Process Manager
echo "⚡ [5/5] Menjalankan layanan dengan PM2 Cluster..."
